...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            //IF activPlayer is equal to 'X', the x.png is placed in HTML
            select.style.backgroundImage = 'url("images/x2.png")';
            //Active player may only be 'X' or 'O' so, if not 'X' iot must be 'O'
        } else {
    ...
    function computersTurn() {
        ...
        while (!success) {
            ...
            //if the random number evaluated returns tru, the square hasn't been selected yet.
...
function checkWinConditions() {
    ...
    // O 0, 1, 2 condition.
    else if (arrayIncludes('0O', '1O', '2O')) { drawWinLine(50, 200, 558, 100) }
    // O 3, 4, 5 condition.
    else if (arrayIncludes('3O', '4O', '5O')) { drawWinLine(50, 304, 558, 305) }
    // O 6, 7, 8 condition.
...
function audio(audioURL) {
    //We create a new sudio object and we pass the path as a paramater.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    let x1 = coordX1,
        ...
        y2 = coordY2,
        //This variable stores temporary x axis data we update in out animation loop.
        x = x1,
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 <= y2) {
            //This condition adds 10 to the previoys end x endpoint.
            if (x < x2) {x += 10; }
